...
public class ContactListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.noticeImave = (ImageView) convertView.findViewById(R.id.noticeImave);
        ...
        List<MessageEntity> listds = new MessageDao().getAllUnReadMessages(Integer.parseInt(contactBean.getUid()));
        Logger.e("listees"+listds.size());
        if(listds.size()==0){
            holder.noticeImave.setVisibility(View.GONE);
        }else{
            for (MessageEntity entity : listds) {
                Logger.e("enter"+entity.getContent());
                if(Integer.parseInt(contactBean.getUid())==entity.getUserId());{
                    holder.noticeImave.setVisibility(View.GONE);
                }
            }
        }
...
//        Picasso.with(context).load(list.get(position).getAvatar_original()).into(holder.contactAvatar);
    ...
    private class ViewHolder {
        ...
        ImageView msgIcon;
        ImageView isChecked;
        ImageView contactAvatar,noticeImave;
    }
