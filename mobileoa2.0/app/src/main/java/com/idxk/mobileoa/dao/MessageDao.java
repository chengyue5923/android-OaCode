...
import com.idxk.mobileoa.utils.cache.db.InsertRequest;
 ...
 */
public class MessageDao extends BaseOaDao {
    public MessageDao() {
        super();
        createTable();
    }
    ...
    public List<MessageEntity> getAllUnReadMessagesBySessionId(String sessionId) {
        ...
        for (Map<String, String> map : maps) {
            Logger.e("==========unreadmap---"+map.toString());
            try {
...
//                model.setLocId(Integer.valueOf(map.get("id")));
    ...
    public List<MessageEntity> getAllUnReadMessages(int uid) {
        List<MessageEntity> res = new ArrayList<MessageEntity>();
        String sql = "select * from message where userId = ? and read = 0";
        List<Map<String, String>> maps = opration.querySql(new SqlRequest(sql, new String[]{uid+""}));
        Logger.e("==========unreadmap---"+maps.toString());
        if (ListUtil.isNullOrEmpty(maps)) {
            return res;
        }
        for (Map<String, String> map : maps) {
