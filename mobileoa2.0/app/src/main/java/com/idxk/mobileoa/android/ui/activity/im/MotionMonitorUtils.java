...
import com.idxk.mobileoa.model.bean.Mobile;
...
//    private Sensor mAccelerometerSensor;
    ...
    public MotionMonitorUtils(MotionMonitorUtilsDelegate delegate) {
        ...
        mSensorManager = (SensorManager) MobileApplication.getInstance().getApplicationContext().getSystemService(Context.SENSOR_SERVICE);
...
//                    if (maxValuex > valuex && valuex >= 0.0 && deltaTime < 4) 
                    if (valuex >= maxValuex - Float.MIN_VALUE && valuex <= maxValuex + Float.MIN_VALUE) {
                        AudioManager audioManager = (AudioManager) MobileApplication.getInstance().getApplicationContext().getSystemService(Context.AUDIO_SERVICE);
